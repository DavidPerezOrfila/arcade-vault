---
source_file: "components/skin/SkinProvider.tsx"
type: "code"
community: "leaderboard.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/leaderboardts
---

# SkinProvider.tsx

## Connections
- [[AsteroidsGame.tsx]] - `imports_from` [EXTRACTED]
- [[BloqueBusterGame.tsx]] - `imports_from` [EXTRACTED]
- [[CaidaGame.tsx]] - `imports_from` [EXTRACTED]
- [[Listener]] - `contains` [EXTRACTED]
- [[SerpentinaGame.tsx]] - `imports_from` [EXTRACTED]
- [[SkinContext]] - `contains` [EXTRACTED]
- [[SkinContextValue]] - `contains` [EXTRACTED]
- [[SkinId]] - `imports` [EXTRACTED]
- [[SkinProvider()]] - `contains` [EXTRACTED]
- [[SkinSelect.tsx]] - `imports_from` [EXTRACTED]
- [[applySkin()]] - `contains` [EXTRACTED]
- [[getServerSnapshot()]] - `contains` [EXTRACTED]
- [[getSnapshot()]] - `contains` [EXTRACTED]
- [[isSkinId()]] - `imports` [EXTRACTED]
- [[layout.tsx]] - `imports_from` [EXTRACTED]
- [[listeners]] - `contains` [EXTRACTED]
- [[nav.tsx]] - `imports_from` [EXTRACTED]
- [[readStoredSkin()]] - `imports` [EXTRACTED]
- [[skins.ts]] - `imports_from` [EXTRACTED]
- [[subscribe()]] - `contains` [EXTRACTED]
- [[useSkin()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/leaderboardts