---
name: spec-impl-game
description: Implements an approved game spec for Arcade Vault. Validates that the state means "Approved" (in any language), creates a git branch named after the spec, switches to it, and starts the implementation step by step with pauses to review diffs. When the implementation finishes, launches two agents in cascade — skin-designer first, then mobile-porter — before the final commit.
disable-model-invocation: true
argument-hint: <NN-spec-name>
allowed-tools: Bash(git status:*), Bash(git branch:*), Bash(git checkout:*), Bash(cat:*), Bash(ls:*)
---

# /spec-impl-game — Implementer of approved specs with post-implementation cascade

Same method as `/spec-impl`, plus a final **Phase 5** that runs the `skin-designer` and
`mobile-porter` agents in cascade over the newly implemented game.
...
When all steps are done I will run the post-implementation cascade
(skin-designer → mobile-porter) before the final commit.
...
in your repo's language) and then run the Phase 5 cascade before the final commit.
```

---

### Phase 5 — Post-implementation cascade (skin-designer → mobile-porter)

Run this phase **after** Phase 4 completes (all steps implemented, acceptance criteria verified,
spec state set to `Implemented`) and **before** making the final commit. Do NOT commit yet.

Launch the two agents **in cascade**: strictly sequential, never in parallel. Wait for each agent
to finish and review its output before launching the next one. Both agents work on the current
branch (`spec-NN-slug`), so their changes land together with the spec implementation.

#### Step 5.1 — skin-designer

Launch the `skin-designer` agent (subagent type: `skin-designer`) with a prompt that includes:

- The implemented game: its slug, engine path (`lib/games/<slug>/game.esm.js`), React wrapper
  (`components/games/<slug>/`), CSS, and actions.
- The task: apply the three skins (`clasico`, `neon`, `retro`) to the new game following the
  skin contract — palette tokens via `PALETTES[skin] ?? PALETTES.clasico` resolved inside the
  engine, no hardcoded entity colors, `initGame(refs, { onGameOver, skin })` backward-compatible,
  dark-mode legibility for HUD/canvas/overlays, per-game `SkinSelect` in the game layout if the
  other playable games have one.
- Constraint: do not touch game mechanics, scoring, or the engine contract (`setTimeout` loop,
  idempotent `destroy`, paired listeners, single `onGameOver(score)`).
- Remind it to update `resources/skins-todo.md` coverage and run `npm run graphify:update`.

Review the agent's report. If it reports failures (lint/build/engine contract), fix or re-run
before continuing. Then confirm to the user:

```
✅ Cascade 1/2 complete: skins applied to <slug>.
Proceeding with mobile audit + fixes.
```

#### Step 5.2 — mobile-porter

Launch the `mobile-porter` agent (subagent type: `mobile-porter`) with a prompt that includes:

- The implemented game: slug, wrapper component, CSS file, player route (`/player/<slug>`),
  game detail route (`/games/<slug>`), and game-over modal.
- An explicit instruction to **apply fixes**, not only audit (its default mode is read-only).
  Scope: CSS and minimal markup only — media queries at 320px/375px/414px, touch targets >= 44px,
  canvas scaling inside the CRT frame, HUD overlay behavior, game-over modal fitting the viewport,
  safe-area/dvh handling. It must still generate/update `resources/mobile-audit.md`.
- Constraint: do not touch engines (`game.esm.js`), skin palettes, business logic, or Supabase.

Review the agent's report. Fix any CRITICAL findings it could not resolve before continuing.

#### Step 5.3 — Verify and commit

After both agents finish:

1. Run `npm run lint` and `npm run build`. Fix real failures; do not hide them.
2. Run `npm run graphify:update` so committed graph artifacts stay current.
3. Make **one final commit** on the branch covering everything: spec implementation + skins +
   mobile fixes + updated resources (`skins-todo.md`, `mobile-audit.md`). Use conventional commit
   format, e.g. `feat: implement <slug> per spec NN-slug (incl. skins + mobile)`.
4. Report to the user:

```
✅ Game <slug> complete.

Spec:     specs/NN-slug.md (Implemented)
Branch:   spec-NN-slug
Cascade:  skin-designer ✅ → mobile-porter ✅
Verify:   lint <result> · build <result>

Ready to merge / open PR when you are.
```
...
```
/spec-impl-game 01-frogger

  Phase 1  →  Finds specs/01-frogger.md
  Phase 2  →  Reads the state → "Approved" (or "Aprobado", etc.) → ✅ continues
  Phase 3  →  git checkout -b spec-01-frogger → shows objective, scope, plan, criteria
  Phase 4  →  Implements step by step with pauses
  Phase 5  →  skin-designer (skins) → mobile-porter (audit + fixes)
              lint + build + graphify:update → one final commit

/spec-impl-game 02-powerups  (state: Draft / Borrador)

  Phase 2  →  Reads the state → "Draft" → ❌ stops
              Shows the standard error message
              Does not create branch, does not touch code, no cascade
```
...
**The cascade only runs after a completed Phase 4.** If the implementation stops early (ambiguity,
user abort, blocked step), do not launch the agents — resume Phase 4 in a later invocation and
run Phase 5 only when all steps are done.
