...
function update(dt) {
  if (gameState.state === 'gameover') {
    ...
    for (let i = 0; i < gameState.particles.length; i++) {
      if (!gameState.particles[i].dead) {gameState.particles[wp++] = gameState.particles[i];}
    }
  ...
  for (let i = 0; i < gameState.bullets.length; i++) {
    if (!gameState.bullets[i].dead) gameState.bullets[w++] = gameState.bullets[i];
  }
  gameState.bullets.length = w;
  w = 0;
  for (let i = 0; i < gameState.particles.length; i++) {
    if (!gameState.particles[i].dead) {gameState.particles[w++] = gameState.particles[i];}
  }
  gameState.particles.length = w;
  w = 0;
  for (let i = 0; i < gameState.powerUps.length; i++) {
    if (!gameState.powerUps[i].dead) {gameState.powerUps[w++] = gameState.powerUps[i];}
  }
  ...
  for (let i = 0; i < gameState.asteroids.length; i++) {
    if (!gameState.asteroids[i].dead) {gameState.asteroids[wa++] = gameState.asteroids[i];}
  }
