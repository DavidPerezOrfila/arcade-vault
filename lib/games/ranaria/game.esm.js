...
function laneConfig(level) {
  const f = Math.pow(SPEED_LEVEL_FACTOR, level - 1);
  return [
    // Carretera (filas 8-12)
    { row: 8, dir: -1, speed: 2.2 * f, type: "car", len: 1, count: 3 },
    { row: 9, dir: 1, speed: 3.6 * f, type: "car", len: 1, count: 3 },
    { row: 10, dir: -1, speed: 2.0 * f, type: "truck", len: 2, count: 3 },
    { row: 11, dir: 1, speed: 2.4 * f, type: "car", len: 1, count: 4 },
    { row: 12, dir: -1, speed: 3.2 * f, type: "truck", len: 3, count: 2 },
    // Río (filas 1-6), de arriba hacia abajo
    { row: 1, dir: 1, speed: 1.1 * f, type: "log", len: 4, count: 3 },
    { row: 2, dir: -1, speed: 1.7 * f, type: "turtle", len: 3, count: 3 },
    { row: 3, dir: -1, speed: 2.1 * f, type: "log", len: 2, count: 4 },
    { row: 4, dir: 1, speed: 0.9 * f, type: "log", len: 3, count: 3 },
    { row: 5, dir: 1, speed: 1.5 * f, type: "turtle", len: 2, count: 4 },
    { row: 6, dir: -1, speed: 1.3 * f, type: "log", len: 3, count: 3 },
  ].map((cfg) => ({
    ...cfg,
...
function mouthIndexAt(col) {
  return MOUTHS.findIndex((pair) => pair[0] === col || pair[1] === col);
}
...
function updateTimeHUD() {
  if (!refs) return;
  refs.timeEl.textContent = `${Math.ceil(timeLeft)}s`;
}
