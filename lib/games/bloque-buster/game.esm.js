...
function update(dt) {
  ...
  // ponytail: in-place compaction, no new array per frame
  let w = 0;
  for (let i = 0; i < explosions.length; i++) {
    if (explosions[i].elapsed < EXPLOSION_DURATION) explosions[w++] = explosions[i];
  }
  explosions.length = w;
...
function drawHUD() {
  ...
  // ponytail: font/textAlign/textBaseline hoisted to initGame — set once, not every tick
...
export function initGame(targetCanvas, options = {}) {
  ...
  canvas.width = W;
  canvas.height = H;
  // ponytail: font + textAlign + textBaseline set once — hoisted from drawHUD
  ctx.font = 'bold 18px monospace';
  ctx.textAlign = 'left';
