...
import type { CaidaGameProps, CaidaRefs, LeaderboardEntry } from '@/lib/games/caida/types';
import './caida.css';

interface GameModule {
  // eslint-disable-next-line no-unused-vars
  initGame: (refs: CaidaRefs, _options?: { onGameOver?: (_score: number) => void }) => void;
  destroy: () => void;
  // eslint-disable-next-line no-unused-vars
...
export function CaidaGame({ initialLeaderboard = [] }: CaidaGameProps) {
  ...
  const startGame = useCallback(() => {
    ...
    if (
      !game || !board || !nextCanvas || !scoreEl || !linesEl || !levelEl ||
    ...
    game.initGame(
      { board, nextCanvas, scoreEl, linesEl, levelEl, overlay, overlayTitle, overlayScore },
  ...
  return (
    <div className='caida-game-container'>
      <div className='caida-board-wrap'>
        <canvas ref={boardRef} width={300} height={600} aria-label='Juego CAÍDA' />

        {/* Overlay compartido: PAUSA / GAME OVER. El wrapper togglea la clase 'hidden'. */}
        <div className='caida-overlay hidden' ref={overlayRef}>
          <h3 className='caida-overlay-title' ref={overlayTitleRef}>PAUSA</h3>
        ...
        {showAuthPrompt && (
          <div className='caida-auth-overlay'>
            <div className='caida-auth-prompt'>
              ...
              <p className='caida-auth-message'>Inicia sesión para guardar tu puntuación en el ranking global.</p>
              <a
        ...
        <div className='caida-leaderboard'>
          ...
          <ol className='caida-leaderboard-list'>
            {leaderboard.slice(0, 10).map((entry) => (
              ...
              >
                <span className='caida-leaderboard-rank-player'>#{entry.rank} {entry.playerName}</span>
